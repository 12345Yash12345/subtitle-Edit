...
const App = () => {
  ...
  const handleVideoUpload = async (event) => {
    try {
      ...
      const videoFileBlob = await fetch(URL.createObjectURL(file)).then((res) => res.blob());
  
      // Reset subtitles when a new video is uploaded
      setSubtitles([]);
  
      // Set the video file blob
      setVideoFile(videoFileBlob);
  
      // Create FormData and append video file and subtitles
      const formData = new FormData();
      formData.append('video', videoFileBlob, 'video.mp4'); // Adjust the filename as needed
      formData.append('subtitles', JSON.stringify(subtitles));
